...
import { anything, instance, mock, verify } from '../utilities/mockito'
...
describe('SchemaService', function () {
    ...
    it('assigns schemas to the yaml extension', async function () {
        ...
        verify(fakeYamlExtension.assignSchema(vscode.Uri.file('/foo').toString(), cfnRegistry, cfnSchema)).once()
        verify(fakeYamlExtension.assignSchema(vscode.Uri.file('/bar').toString(), samRegistry, samSchema)).once()
    })
    ...
    it('removes schemas from the yaml extension', async function () {
        ...
        await service.processUpdates()
        verify(fakeYamlExtension.removeSchema(vscode.Uri.file('/foo').toString(), samRegistry)).once()
    })
